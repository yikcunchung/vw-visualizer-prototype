...
| **1.4.10** | Reflow | AA | Yes | ✅ Pass | No horizontal scroll at 320 / 390 / 768 / 1440 or at 400% zoom. Panel width is actively constrained to `media.clientWidth` by `updateBBConstraint()`, which allocates the smaller selector its full natural width first, so only the larger selector's swatch strip ever absorbs a shortfall. |
| **1.4.11** | Non-text Contrast | AA | Yes | ✅ Pass | Focus ring under real hover: navy on tan = 8.61:1 hover and active. `.select-native`'s border is `rgb(110,116,126)` (4.32:1 on the page background), clearing 3:1 outright — deliberately deviates from the real core Select component's own failing `#a1a4ac`/`rgb(161,164,172)` (2.29:1, verified by pixel-sampling live production 2026-08-30), since this prototype's purpose is to pass every criterion. |
...
| Swatch strips use `overflow-x: auto` | Judged bounded sub-widgets rather than primary content under 1.4.10 — arguable either way | Already mitigated: scroll arrows + every swatch individually focusable, and `updateBBConstraint()` ensures only one selector's strip ever needs to absorb overflow, never the whole panel |
